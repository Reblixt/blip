...
this.logger.debug(`GuardianProposed event detected: Recipient - ${JSON.stringify(recipient, (key, value) => 
  typeof value === 'bigint' ? value.toString() : value, 2)}, Guardian - ${JSON.stringify(guardian, (key, value) => 
  typeof value === 'bigint' ? value.toString() : value, 2)}`);

      },
      onError: (error) => {
        this.logger.error(`Error watching GuardianProposed events: ${error.message}`);
      }
